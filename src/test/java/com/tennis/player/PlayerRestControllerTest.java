...
class PlayerRestControllerTest {
	...
	void shouldGetPlayerById() throws Exception {
		...
		mockMvc.perform(get(PLAYER_ENDPOINT + "/2")).andExpect(status().isOk())
				.andExpect(MockMvcResultMatchers.content().contentType("application/hal+json"))
	...
	void shouldGetErrorResponse() throws Exception {
		...
		mockMvc.perform(get(PLAYER_ENDPOINT + "/50")).andExpect(status().isNotFound())
	...
	void shouldGetBadRequest() throws Exception {
		...
		mockMvc.perform(get(PLAYER_ENDPOINT + "/mm")).andExpect(status().isBadRequest())
				.andExpect(MockMvcResultMatchers.content().contentType("application/json"));
	}
	...
	void shouldGetStatistic() throws Exception {
		...
		mockMvc.perform(get(PLAYER_ENDPOINT + "/statistics")).andExpect(status().isOk())
				.andExpect(MockMvcResultMatchers.content().contentType("application/hal+json"))
				.andExpect(jsonPath("$.response.imcMean", is(23.36)))
				.andExpect(jsonPath("$.response.playerHeightMedian", is(185.0)))
				.andExpect(jsonPath("$.response.countryWithBigRatio.code", is("SRB")))
